...
void rotate(float alpha1, float alpha2, float w)
{
    float a1, a2;
    int ll;
    if (fabs(alpha1) > fabs(alpha2)) { a1 = 1; ll = alpha1 * 1; a2 = a1 * alpha2/alpha1; }
    else if (fabs(alpha1) < fabs(alpha2)) { a2 = 1; ll = alpha2 * 1; a1 = a2 * alpha1/alpha2; }
    else if (fabs(alpha1) == fabs(alpha2)) { a1 = 1; a2 = 0.1; ll = alpha1 * 1; }
...
int main()
{
    ...
    while(1)
    {
        ...
        //printf("pulse1: %d, pulse2: %d, row_col: %d\n", atoi(row[0]), atoi(row[1]), atoi(row[2]));
        
        
        if (atoi(row[2])!=0 && atoi(row[4])==1)
        {
            float nrow = atoi(row[2])/10;
            float ncol = atoi(row[2])%10;
            printf("x_o: %d", atoi(row[3]));
            if (atoi(row[3])==0)
            {
                draw_x(nrow, ncol);
            }
    ...
    // for (int xx=1; xx<chessboard_size+1; xx++)
    // {
    //     for (int yy=1; yy<chessboard_size+1; yy++)
    //     {
    //         if ((xx+yy)%2==0)
    //         {
    //             draw_o(xx, yy);
